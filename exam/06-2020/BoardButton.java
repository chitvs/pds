 ...
 *
 * <p>
 * Each {@code BoardButton} may or may not contain a mine, depending on the
 * value of the <i>mine</i> property. When the game starts, the player can start
 * toggling buttons in the board. If the toggled button contains a mine, then
 * the player loses and the game ends, otherwise the button sets its text to the
 * number of directly adjacent cells (including diagonals) containing a mine.
...
public class BoardButton extends JToggleButton {
	...
	/**
	 * Reset the button to the initial state. You <strong>must</strong> call this
	 * method and NOT {@code setSelected(false)} if you want to reset the button to
	...
	/**
	 * Reveal the content of the cell. This is equivalent of calling
	 * {@code setSelected(true)} on the button.
	 */
	public void reveal() {
		setSelected(true);
	}
	...
	/**
	 * Subclass of {@link JToggleButton.ToggleButtonModel} that implements a
	 * one-time only selection behaviour for the button.
	 */
